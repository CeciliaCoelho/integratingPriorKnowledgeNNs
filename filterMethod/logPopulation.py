...
# Define the ODE function to generate the data
true_y0 = torch.tensor([2.518629]).to(device)

t = torch.linspace(0.0, args.tf, args.data_size).to(device)
t_test = torch.linspace(0.0, args.tf_test, args.test_data_size).to(device)
...
# Define the NN to learn the ODE
...
if __name__ == "__main__":
    ...
    for itr in range(1, args.niters + 1):
        ...
        violation = torch.mean(
            (
                torch.maximum(
                    torch.subtract(pred_y_clone, 12), torch.Tensor([0]).to(device)
                )
            )
        )
        ...
        trialF_violation = torch.mean(
            (
                torch.maximum(
                    torch.subtract(trialF_pred_y, 12), torch.Tensor([0]).to(device)
                )
            )
        )
        ...
        trialV_violation = torch.mean(
            (
                torch.maximum(
                    torch.subtract(trialV_pred_y, 12), torch.Tensor([0]).to(device)
                )
            )
        )
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                if itr == args.niters:
                    ...
                    violation_t = torch.mean(
                        (
                            torch.maximum(
                                torch.subtract(pred_y_test, 12),
                                torch.Tensor([0]).to(device),
                            )
                        )
                    )
                    print(
                        "MSE Loss {:.6f} | Violation {:.6f}".format(
                            mse_t.item(), violation_t.item()
                        )
                    )
                    plt.plot(
