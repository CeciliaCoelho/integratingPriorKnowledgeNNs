...
if __name__ == "__main__":
    ...
    for itr in range(1, args.niters + 1):
        ...
        # Clone pred_y before updating parameters to preserve its computation graph
        pred_y_clone = pred_y.clone().detach().requires_grad_(True)
        violation = torch.mean(
            torch.square(torch.subtract(torch.sum(pred_y_clone, dim=1), tot_m))
        )
        ...
        trialF_violation = torch.mean(
            torch.square(torch.subtract(torch.sum(trialF_pred_y, dim=1), tot_m))
        )
        ...
        trialV_violation = torch.mean(
            torch.square(torch.subtract(torch.sum(trialV_pred_y, dim=1), tot_m))
        )
        ...
        if trialF_violation <= tolF and trialV_violation <= tolF:
            ...
            filterT, acceptF = filterMethod(
                filterT, trialF_params, trialF_loss, trialF_violation
            )
            filterT, acceptV = filterMethod(
                filterT, trialV_params, trialV_loss, trialV_violation
            )
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                pen = torch.mean(
                    torch.square(torch.subtract(torch.sum(pred_y, dim=1), tot_m))
                )
                print(
                    "Iter {:04d} | MSE Loss {:.6f} | Penalty {:.6f}".format(
                        itr, loss.item(), pen.item()
                    )
                )
                ...
                if itr == args.niters:
                    ...
                    pen = torch.mean(
                        torch.square(
                            torch.subtract(torch.sum(pred_y_test, dim=1), tot_m)
                        )
                    )
                    print(
                        "MSE Test {:.6f} | Violation {:.6f}".format(
                            mse.item(), pen.item()
                        )
                    )
                    plt.plot(
                    ...
                    plt.plot(
                        times_test,
                        pred_y_test[:, 0].detach().cpu().numpy(),
                        label="A",
                        color="green",
                    )
                    plt.plot(
                        times_test,
                        pred_y_test[:, 1].detach().cpu().numpy(),
                        label="B",
                        color="blue",
                    )
                    plt.plot(
                        times_test,
                        pred_y_test[:, 2].detach().cpu().numpy(),
                        label="C",
                        color="orange",
                    )
                    plt.plot(
                        times_test,
                        pred_y_test[:, 3].detach().cpu().numpy(),
                        label="D",
                        color="red",
                    )
                    plt.xlabel("Time (s)")
                    ...
                    conservation = [
                        abs(total_mass[i] - total_mass[0])
                        for i in range(1, len(total_mass))
                    ]
                    print(
                        "Conservation of mass violated ",
                        sum(list(map(lambda x: x > 1e-6, conservation))).item(),
                        " times!",
                    )
